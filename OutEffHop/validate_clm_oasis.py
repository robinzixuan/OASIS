...
from run_clm_oasis import get_decoder_components, replace_attention_modules, patch_model_forward_for_oasis
...
def main():
    ...
    # >> replace self-attention module with OASIS attention (supports OPT, Llama, Qwen, Phi-4)
    decoder_info = replace_attention_modules(model, args)

    # >> patch model forward to propagate layer_outputs_history and null_posteriors_history
    patch_model_forward_for_oasis(model)

    # >> reload ALL weights (including OASIS-specific attn_res_sa, attn_res_mlp, oasis_beta_raw)
    #    AutoModelForCausalLM.from_pretrained drops these since vanilla Llama doesn't have them.
    ...
    if os.path.exists(st_single):
        ...
        for shard in set(weight_map.values()):
            state_dict.update(load_safetensors(os.path.join(args.model_name_or_path, shard)))
    elif os.path.exists(bin_path):
        state_dict = torch.load(bin_path, map_location="cpu")
    if state_dict:
        missing, unexpected = model.load_state_dict(state_dict, strict=False)
        if missing:
            logger.info(f"Reloaded weights — {len(missing)} missing keys (new/unused params)")
        if unexpected:
            logger.warning(f"Reloaded weights — {len(unexpected)} unexpected keys")
    ...
    #
    if args.quantize:
        from transformers_language.models.quantized_llama_oasis import QuantizedLlamaOasisForCausalLM
        from transformers_language.models.quantized_qwen_oasis import QuantizedQwen3ForCausalLM
        ...
        if decoder_info["arch"] == "opt":
            ...
            model = QuantizedLlamaOasisForCausalLM(model, **qparams)
