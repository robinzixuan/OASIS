...
from transformers_language.models.qwen_oasis_attention import (
    Qwen3AttentionWithExtras,
    Qwen3DecoderLayerExtra,
    AttentionResidual,
    _apply_softmax_fn,
    _renormalize_if_needed,
)


class QuantizedQwen3AttentionWithExtras(QuantizedModel):
    def __init__(self, org_model, **quant_params):
        super().__init__()

        self.config = org_model.config
        self.layer_idx = org_model.layer_idx
        self.layer_type = org_model.layer_type
        ...
        self.sliding_window = org_model.sliding_window
        ...
        self.q_norm = org_model.q_norm
        self.k_norm = org_model.k_norm
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_weights = _apply_softmax_fn(attn_weights, self.softmax_fn, dim=-1)
        attn_weights = _renormalize_if_needed(attn_weights, dim=-1)

        # OASIS: null posterior = mass routed to null space
        ...
        # OASIS: branch-level null = mean across heads
        branch_null = null_posterior.mean(dim=1)  # (B, T)
...
class QuantizedQwen3DecoderLayerExtra(QuantizedModel):
    def __init__(self, org_model, **quant_params):
        super().__init__()
        ...
        self.self_attn = QuantizedQwen3AttentionWithExtras(org_model.self_attn, **quant_params)
...
class QuantizedQwen3Model(QuantizedModel):
    def __init__(self, org_model, **quant_params):
        super().__init__()
        ...
        self.layers = quantize_model(
            org_model.layers,
            specials={Qwen3DecoderLayerExtra: QuantizedQwen3DecoderLayerExtra},
            **quant_params,
        )
...
class QuantizedQwen3ForCausalLM(QuantizedModel, PreTrainedModel):
    def __init__(self, org_model, quant_setup=None, **quant_params):
        ...
        self.model = QuantizedQwen3Model(org_model.model, **quant_params)
