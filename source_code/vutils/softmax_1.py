...
def softmax_n_shifted_zeros(input: torch.Tensor, n: int, dim=-1, dtype=torch.float32) -> torch.Tensor:
    ...
    # Include the null logit (0) in the max to prevent exp overflow when all inputs are very negative.
    # Without this, if max(x) < -88, then exp(-max(x)) overflows to inf in float32 -> NaN.
    input_maxes = torch.maximum(
        input.max(dim=dim, keepdim=True).values,
    ...
    # the null logit (0) shifted by the same max
    null_term = torch.exp(-input_maxes) * n
    denominator = original_denominator + null_term
...
def softmax_1(input: torch.Tensor, dim=-1, dtype=torch.float32) -> torch.Tensor:
    ...
    """
    return softmax_n_shifted_zeros(input, 1, dim=dim, dtype=dtype)
...
class Softmax_1(nn.Module):
    ...
    def forward(self, input):
        a = softmax_1(input, self.dim, dtype=dtype) 
        return a
