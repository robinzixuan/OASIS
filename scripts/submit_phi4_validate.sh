...
# Submit: sbatch /path/to/OASIS/scripts/submit_phi4_validate.sh
#===============================================================================
...
# Slurm ??????/var/spool/slurmd/...???? BASH_SOURCE ??????????cwd??if [[ -n "${SLURM_SUBMIT_DIR:-}" ]]; then
  if [[ -d "${SLURM_SUBMIT_DIR}/source_code" ]]; then
    ...
    echo "source_code not found under SLURM_SUBMIT_DIR=${SLURM_SUBMIT_DIR}; run sbatch from repo root (parent of source_code) or from scripts."
    exit 1
  fi
else
